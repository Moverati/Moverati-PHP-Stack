...
class WURFL_Xml_PersistenceProvider_PersistenceProviderManager {
	...
	private static function _initialize($persistenceConfig) {
        ...
        /**
         * @todo PATCHED BY DANIEL COUSINEAU
         */
        $params   = array();
        if( isset($persistenceParams["params"]) )
            $params = $persistenceParams["params"];
		
		switch ($provider) {
			case WURFL_Constants::MEMCACHE :
				self::$_persistenceProvider = new WURFL_Xml_PersistenceProvider_MemcachePersistenceProvider ( $params );
				break;
			case WURFL_Constants::APC :
				self::$_persistenceProvider = new WURFL_Xml_PersistenceProvider_APCPersistenceProvider ( $params );
				break;
			case WURFL_Constants::MYSQL :
				self::$_persistenceProvider = new WURFL_Xml_PersistenceProvider_MysqlPersistenceProvider ( $params );
				break;
			default :
				self::$_persistenceProvider = new WURFL_Xml_PersistenceProvider_FilePersistenceProvider ( $params );
				break;
		}
	}
